...
def generate_personalized_response(query):
    ...
    try:
        ...
        text_results = search_text_chunks(formatted_query)
        image_results = search_images(formatted_query)
...
tab1, tab2, tab3 = st.tabs(["Chat Interface", "Upload Documents", "Learning Analytics"])
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                if st.session_state.show_sources:
                    text_results = search_text_chunks(prompt)
                    text_context = "\n\n".join([
                        f"Source: {result['module_code']}, Lecture {result.get('lecture_number', 'Unknown')}, Page {result['page']}\n"
                        f"Content: {result['content']}"
                        for result in text_results
